...
auto LRUKReplacer::Evict(frame_id_t *frame_id) -> bool {
  std::scoped_lock<std::mutex> lock(latch_);  // 大锁

  if (curr_size_ == 0) {  // 可驱逐页面数为0
    return false;
  }

  for (auto it = history_list_.rbegin(); it != history_list_.rend(); it++) {  // 在历史队列里面剔除
    auto frame = *it;
    if (is_evictable_[frame]) {  // 页面可剔除时
  ...
  for (auto it = cache_list_.rbegin(); it != cache_list_.rend(); it++) {  // 在缓存队列里面剔除
    auto frame = *it;
...
void LRUKReplacer::RecordAccess(frame_id_t frame_id) {
  ...
  if (access_count_[frame_id] == k_) {
    ...
    if (history_map_.count(frame_id) == 0U) {  // bug free
...
void LRUKReplacer::SetEvictable(frame_id_t frame_id, bool set_evictable) {
  ...
  if (!is_evictable_[frame_id] && set_evictable) {  // 不可驱逐变可驱逐
    curr_size_++;
  }

  if (is_evictable_[frame_id] && !set_evictable) {  // 可驱逐变不可驱逐
    curr_size_--;
  }
