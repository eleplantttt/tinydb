...
template <typename K, typename V>
auto ExtendibleHashTable<K, V>::CalIndex(const size_t &idx, int depth) -> size_t {
...
auto ExtendibleHashTable<K, V>::Expansion() -> void {
  ...
  for (size_t i = old_size; i < new_size; ++i) {  // 从后一半开始指针重定向old_size开始
    dir_.push_back(dir_[CalIndex(i, global_depth_)]);  // eg:从i=old_size=4(100) 全局深度已经加一为3 传入 传出dir[4]
  }
}
